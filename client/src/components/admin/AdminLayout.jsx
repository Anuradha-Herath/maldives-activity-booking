...
const AdminLayout = ({ children }) => {
  ...
  const navLinks = [
    ...
    { path: '/admin/settings', icon: 'fa-cog', text: 'Settings' },
  ];
    ...
    <div className="flex h-screen bg-gray-100">
          ...
          <div className="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
            ...
            <nav className="mt-5 px-2 space-y-1">
              {navLinks.map((link) => (
                ...
                >
                  <i className={`fas ${link.icon} mr-4 text-blue-300`}></i>
                  {link.text}
                </Link>
              ))}
            </nav>
          ...
          <div className="flex-shrink-0 flex border-t border-blue-700 p-4">
            <Link to="/" className="flex-shrink-0 group block">
              <div className="flex items-center">
                <div>
                  <span className="inline-block h-10 w-10 rounded-full bg-blue-600 flex items-center justify-center">
      ...
      <div className="hidden lg:flex lg:flex-shrink-0">
        <div className="flex flex-col w-64">
          <div className="flex flex-col h-0 flex-1 bg-blue-800 text-white">
            <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
              ...
              <nav className="mt-5 flex-1 px-2 space-y-1">
                {navLinks.map((link) => (
                  ...
                  >
                    <i className={`fas ${link.icon} mr-3 text-blue-300`}></i>
                    {link.text}
                  </Link>
                ))}
              </nav>
            ...
            <div className="flex-shrink-0 flex border-t border-blue-700 p-4">
              <Link to="/" className="flex-shrink-0 w-full group block">
                <div className="flex items-center">
                  <div>
                    <span className="inline-block h-9 w-9 rounded-full bg-blue-600 flex items-center justify-center">
                      <i className="fas fa-user text-blue-200"></i>
                    </span>
                  </div>
                  <div className="ml-3">
